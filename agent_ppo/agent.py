...
# Hero-specific skill strategy / 英雄-召唤师技能策略
# 112=鲁班七号, 133=狄仁杰
# (my_hero, opponent_hero) -> skill_id 确定性对阵策略
MATCHUP_SKILL_MAP = {
    (112, 112): 80110,  # 鲁班 mirror: 狂暴对点
    (112, 133): 80115,  # 鲁班 vs 狄仁杰: 闪现躲大招
    (133, 112): 80115,  # 狄仁杰 vs 鲁班: 闪现追击
    (133, 133): 80102,  # 狄仁杰 mirror: 治疗术（二技能自带解控，净化冗余）
}
...
class Agent(BaseAgent):
    def __init__(self, agent_type="player", device=None, logger=None, monitor=None):
        ...
        self.optimizer = torch.optim.AdamW(params=parameters, lr=self.lr, betas=(0.9, 0.999), eps=1e-8, weight_decay=Config.WEIGHT_DECAY)
    ...
    def lr_lambda(self, step):
        # Warmup + cosine decay: linear 0→1 for first WARMUP_STEPS, then cosine to target_lr/lr
        # Warmup + cosine 衰减: 前 WARMUP_STEPS 步线性增长，之后余弦衰减到 target_lr/lr
        warmup = Config.WARMUP_STEPS
        if step < warmup:
            return step / warmup
        progress = (step - warmup) / max(self.target_step - warmup, 1)
        progress = min(progress, 1.0)
        cos_decay = 0.5 * (1.0 + math.cos(math.pi * progress))
        ratio = self.target_lr / self.lr
    ...
    def predict(self, observation):
        try:
            ...
            import traceback
            self.logger.error(f"[DIAG] predict failed: {type(e).__name__}: {e}")
            self.logger.error(f"[DIAG] traceback: {traceback.format_exc()}")
            raise
    ...
    def exploit(self, observation):
        try:
            ...
            import traceback
            self.logger.error(f"[DIAG] exploit failed: {type(e).__name__}: {e}")
            self.logger.error(f"[DIAG] traceback: {traceback.format_exc()}")
            raise
